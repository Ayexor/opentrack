...
int main(void)
{
    ShmPosix lck_posix(PROTON_SHM_NAME, PROTON_MTX_NAME, sizeof(ProtonSHM));
    ...
    ProtonSHM* shm_posix = (ProtonSHM*) lck_posix.ptr();
    FTHeap* shm_proton = (FTHeap*) lck_wine.ptr();
    FTData* data = &shm_proton->data;
    create_registry_key();
    while (1) {
        if (shm_posix->stop)
            break;
        ...
        data->CamWidth = 250;
        data->CamHeight = 100;
        shm_proton->GameID2 = shm_posix->gameid2;
        shm_posix->gameid = shm_proton->GameID;
        for (int i = 0; i < 8; i++)
            shm_proton->table[i] = shm_posix->table[i];
        (void) Sleep(4);
    }
}
